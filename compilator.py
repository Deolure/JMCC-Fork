...
_builtin_context = None
_loading_builtins = False

def _get_builtin_context():
    global _builtin_context, _loading_builtins
    if _builtin_context is None and not _loading_builtins:
        _loading_builtins = True
        import os as _os2
        _ops_path = _os2.path.join(_os2.path.dirname(_os2.path.abspath(__file__)), "all_operations.jc")
        with open(_ops_path, "r", encoding="UTF-8") as _f:
            _builtin_context = parse_from_file(_f)
        _loading_builtins = False
    return _builtin_context

def _inject_builtins(source):
    if Context(source).get_property("load_builtins", True) is False:
        return
    builtin = _get_builtin_context()
    if builtin is None:
        print("[DEBUG] _get_builtin_context() returned None")
        return
    b_src = builtin.source
    b_callable = builtin.context[b_src][0]["callable"]
    Context(source).context[source][0]["callable"].update(b_callable)
...
class Parser:
    ...
    def factor(self):
        ...
        elif token.type == Tokens.JMCC_DEFINE:
            self.eat(Tokens.JMCC_DEFINE)
            if self.current_token.type == Tokens.DOUBLE_COLON:
                self.eat(Tokens.DOUBLE_COLON)
                act = self.current_token
                self.eat(Tokens.VARIABLE)
                if act.value == "run_function":
                    self.eat(Tokens.LPAREN)
                    raw_args = self.get_args()
                    ending_pos = self.current_token.ending_pos
                    self.eat(Tokens.RPAREN)
                    positional = raw_args.positional
                    file_name = positional[0] if len(positional) > 0 else None
                    fun_name  = positional[1] if len(positional) > 1 else None
                    call_args = positional[2] if len(positional) > 2 else lst([], token.starting_pos, ending_pos, token.source)
                    return run_python_function(None, file_name, fun_name, call_args,
                                               token.starting_pos, ending_pos, token.source)
                elif act.value == "error":
                    self.eat(Tokens.LPAREN)
                    raw_args = self.get_args()
                    ending_pos = self.current_token.ending_pos
                    self.eat(Tokens.RPAREN)
                    positional = raw_args.positional
                    msg = positional[0].value if len(positional) > 0 else ""
                    error("CompileError", msg, token.starting_pos, ending_pos, token.source)
                else:
                    error_from_object(act, "SyntaxError", translate("error.syntaxerror.unexpected_token",
                                                                    {0: act.value}))
            else:
        ...
        if self.current_token.type == Tokens.JMCC_DEFINE:
            ...
            if self.current_token.type == Tokens.DOUBLE_COLON:
                ...
                if act.value == "error":
                    ...
                    positional = raw_args.positional
                    msg = positional[0].value if len(positional) > 0 else ""
                    error("CompileError", msg, act.starting_pos, ending_pos, act.source)
                else:
                    error_from_object(act, "SyntaxError", translate("error.syntaxerror.unexpected_token",
                                                                    {0: act.value}))
...
class run_python_function:  # is_jmcc_object
    type = "run_python_function"
    __slots__ = ("variable", "file_name", "fun_name", "args", "value_type", "starting_pos", "ending_pos", "source")

    def __init__(self, variable, file_name, fun_name, args, starting_pos, ending_pos, source):
    ...
    def simplify(self, mode=None, work_with=None):
        ...
        # Вызываем питон-функцию прямо сейчас во время компиляции
        spec = importlib.util.spec_from_file_location("mod", self.file_name.value)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        fn = getattr(mod, self.fun_name.value)
        result = fn(*[a.value for a in self.args.values])

        def to_jmcc(val):
            if isinstance(val, str):
                return text(val, Texts.LEGACY, self.starting_pos, self.ending_pos, self.source)
            elif isinstance(val, (int, float)):
                return number(val, self.starting_pos, self.ending_pos, self.source)
            elif isinstance(val, list):
                return lst([to_jmcc(v) for v in val], self.starting_pos, self.ending_pos, self.source)
            elif isinstance(val, dict):
                keys = [to_jmcc(k) for k in val.keys()]
                vals = [to_jmcc(v) for v in val.values()]
                return dct(keys, vals, self.starting_pos, self.ending_pos, self.source)
            else:
                error_from_object(self, "TypeError", f"run_function: неподдерживаемый тип {type(val)}")

        return [], to_jmcc(result), []

    def copy(self):
        return run_python_function(
            self.variable, self.file_name, self.fun_name,
            self.args, self.starting_pos, self.ending_pos, self.source
        )
...
def try_upload_tmpfiles(code):
    try:
        print(minecraft_based_text(translate("compilator.code_uploading_message.tmpfiles")))
        start_time1 = time()
        data = json.dumps(code, separators=(',', ':')).encode("utf-8")
        response = requests.post(
            'https://tmpfiles.org/api/v1/upload',
            files={"file": ("code.json", data, "application/json")}
        )
        response = response.json()
        end_time1 = time()
        print(minecraft_based_text(translate("compilator.code_uploading_time", {0: round(end_time1 - start_time1, 3)})))
        url = response.get("data", {}).get("url", "")
        # tmpfiles.org returns https://tmpfiles.org/XXXXX/code.json — convert to direct link
        direct = url.replace("tmpfiles.org/", "tmpfiles.org/dl/") if url else "?"
        print(minecraft_based_text(translate("compilator.code_uploaded_message.tmpfiles", {0: direct})))
    except Exception as e:
        print(minecraft_based_text("&c" + translate("compilator.uploading_error_message")))
        print(error("UploadingError", translate(f"error.uploadingerror.{e.__class__.__name__}",
                                                {i: e.args[i] for i in range(len(e.args))})))
...
def try_upload_litterbox(code, time_val="1h"):
    allowed = {"1h", "12h", "24h", "72h"}
    if time_val not in allowed:
        time_val = "1h"
    try:
        print(minecraft_based_text(translate("compilator.code_uploading_message.litterbox")))
        start_time1 = time()
        data = json.dumps(code, separators=(',', ':')).encode("utf-8")
        response = requests.post(
            'https://litterbox.catbox.moe/resources/internals/api.php',
            data={"reqtype": "fileupload", "time": time_val},
            files={"fileToUpload": ("code.json", data, "application/json")}
        )
        url = response.text.strip()
        end_time1 = time()
        print(minecraft_based_text(translate("compilator.code_uploading_time", {0: round(end_time1 - start_time1, 3)})))
        print(minecraft_based_text(translate("compilator.code_uploaded_message.litterbox", {0: url, 1: time_val})))
    except Exception as e:
        print(minecraft_based_text("&c" + translate("compilator.uploading_error_message")))
        print(error("UploadingError", translate(f"error.uploadingerror.{e.__class__.__name__}",
                                                {i: e.args[i] for i in range(len(e.args))})))
...
def try_upload(code, upload_target=None, upload_time=None):
    if upload_target is None:
        upload_target = "justmc"
    upload_target = str(upload_target).lower().strip()
    if upload_target == "justmc":
        try_upload_justmc(code)
    elif upload_target == "tmpfiles":
        try_upload_tmpfiles(code)
    elif upload_target == "litterbox":
        try_upload_litterbox(code, time_val=upload_time or "1h")
    else:
        print(minecraft_based_text("&c" + translate("compilator.uploading_error_message.unknown_target", {0: upload_target})))
...
def compile_file(file_path, upload=False, properties=None):
    ...
    if upload:
        try_upload(code,
                   upload_target=properties.get("upload_target", "justmc"),
                   upload_time=properties.get("upload_time", "1h"))
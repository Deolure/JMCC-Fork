...
3. [Синтаксис](#синтаксис)
   ...
   - [JMCC Fork Additions](#jmcc-fork-additions)
...
#### JMCC Fork Additions
Было добавлено действия вызова python функции. Действие сохраняет результат python функции в inline переменную:
...
inline var data = jmcc::run_function("data.py", "get_data"); // строка "data"
game var gData = data;
```
В python функцию также можно передавать аргументы:
```py
# main.py
...
```
```ts
inline var sum = jmcc::run_function("main.py", "sum", [1, 2]) // число 3
game var gSum = sum
```
Значение в inline переменную устанавливается только в ПРОЦЕССЕ КОМПИЛЯЦИИ, JustMC не умеет запускать python файлы.

Ещё я добавил действие вызова ошибки компиляции, оно может пригодиться в предупреждении пользователя во время использовании какой либо функции, где некоторый функционал не предусмотрен:
```ts
if (5 / 0) {

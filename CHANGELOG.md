## JMCC Fork 03/05/2026
Добавлено:
 - Компилятор теперь пытается предугадать какой тип значения использует пользователь
 - Компилятор стал менее жесток с типами
 - Цикл for и while (подробнее в [README.md](README.md))

Исправлено:
 - Операторы и условные операторы теперь поддерживаются в цикле по условию

## JMCC Fork
Добавлено:
 - Версия использует свежую GitHub версию оригинала, так что были добавлены некоторые новвоведения по типу поддержки line переменных
 - Добавлена нативная поддержка операторов +, -, \, * и.т.д, а поддержка операторов сравнения ==, >=, > и.т.д (отключить можно в jmcc.properties, ключ - load_builtins)
 - Добавлено действие запуска python функции и сохранения её возвращаемого значения в inline переменную (подробнее в [README.md](README.md))
 - Добавлено действие вызова ошибки компиляции
 - Добавлены иные способы загрузки на удаленный сервер (justmc - по умолчанию, tmpfile, litterbox)

Исправлено:
 - Добавил недостающие ключи локализации (в оригинале почему-то их не было)

...
def download_latest_release(reload=False):
    global data, an_data
    url = requests.get("https://api.github.com/repos/Deolure/JMCC-Fork/releases/latest").json()["assets"][-1][
...
def download_latest_version(reload=False):
    global data, an_data
    filereq = requests.get('https://raw.githubusercontent.com/Deolure/JMCC-Fork/master/release.zip')
    open("release.zip", "wb").write(filereq.content)
    if os.path.isfile("jmcc.properties"):
        with zipfile.ZipFile(f"release.zip", "a") as z:
            for zip_info in z.infolist():
...
def download_latest_data():
    ...
    open("data/actions.json", "w+").write(
        requests.get('https://raw.githubusercontent.com/Deolure/JMCC-Fork/master/data/actions.json').text)
    open("data/events.json", "w+").write(
        requests.get('https://raw.githubusercontent.com/Deolure/JMCC-Fork/master/data/events.json').text)
    open("data/values.json", "w+").write(
        requests.get('https://raw.githubusercontent.com/Deolure/JMCC-Fork/master/data/values.json').text)
    data["data_version"] = an_data["data_version"]
    data.save_properties(open("jmcc.properties", "w+", encoding="UTF-8"))
...
def check_updates():
    ...
    an_data = Properties(text=requests.get(
        'https://raw.githubusercontent.com/Deolure/JMCC-Fork/master/jmcc.properties').text)
